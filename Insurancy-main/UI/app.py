...
def calculate_insurance_score(age, gender, bmi, children, smoke):
...
def service():
    ...
    con = dbConnection()
    cursor = con.cursor()
    cursor.execute('SELECT * FROM registration WHERE Id = %s ', ("1"))
    ...
    if request.method == 'POST':
       ...
       insurance_score = calculate_insurance_score(int(age), int(gender), int(bmi), int(childrence), int(smoke))
       print("Health Insurance Score:", insurance_score)  
        
      
       return jsonify({'pred': str(pred), 'massage': message,'insurance_score': insurance_score})
